...
namespace DNA {
	constexpr double PITCH = 720.0 / 21.0,													// degrees
		STEP = 0.334,
		RADIUS = 1.0,
		SPHERE_RADIUS = 0.13,																// not dna properties, but visual
		OPPOSITE_ROTATION = 155.0;

	constexpr double HALF_TURN_LENGTH = 180.0 / PITCH * STEP;
	constexpr double FULL_TURN_LENGTH = 360.0 / PITCH * STEP;
	constexpr int APPROXIMATION_RADIUS_MULTIPLIER = 4;										// when approximating the cylinder in PhysX, create these exaggerated end bases to compensate for the capsule shape, where their radius is this multiplier times the base RADIUS.
	constexpr double RADIUS_PLUS_SPHERE_RADIUS = RADIUS + SPHERE_RADIUS;
